...
test.describe("progress rendering", () => {
  test("progress bar is visible with correct data-slot", async ({ page }) => {
    ...
    const progress = page.locator('[data-slot="preview"] [data-slot="progress"]').first();
  ...
  test("indicator sub-component is visible", async ({ page }) => {
    await gotoAndWait(page);
    const indicator = page.locator('[data-slot="preview"] [data-slot="progress-indicator"]').first();
